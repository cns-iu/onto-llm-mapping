...
subject_concepts_mappable <- length(unique(evaluative_mappings[evaluative_mappings$mappability=="Mappable",]$subject_id))
subject_concepts_mappable_as <- length(unique(evaluative_mappings[evaluative_mappings$mappability=="Mappable" &
                                                                  evaluative_mappings$mesh_concept_group=="Anatomical structure",]$subject_id))
subject_concepts_mappable_ct <- length(unique(evaluative_mappings[evaluative_mappings$mappability=="Mappable" & 
                                                                  evaluative_mappings$mesh_concept_group=="Cell type",]$subject_id))

# Number of concept mapping records - recalls.
...
# Set F-score beta
beta = 10
# Evaluate hit-miss, MRR, and MAC values for mapping results.
i=2
...
#for(i in 1:length(llm_mapping_paths)){
  ...
  results_subject_concepts_mapped <- 
    ...
    ddply(.(model), summarise,
          mappable_concepts = subject_concepts_mappable,
          mapped_concepts = length(subject_id),
          skipped_concepts = subject_concepts_mappable - length(subject_id),
          mapped_concepts_percent = round(length(subject_id)/subject_concepts_mappable,3))
  ...
  results_hit_miss <-
    ...
    pivot_wider(id_cols = model, 
                ...
                values_from = value,
                values_fn = length) %>%
    mutate(hit_percent = round(Hit/subject_concepts_mappable,2),
           miss_percent = round(Miss/subject_concepts_mappable,2))
  ...
  results_hit_miss_groups <-
    results_hit_miss_groups %>%
    mutate(hit_as_percent= round(hit_as/subject_concepts_mappable_as,2), 
           miss_as_percent= round(miss_as/subject_concepts_mappable_as,2),
           hit_ct_percent= round(hit_ct/subject_concepts_mappable_ct,2),
           miss_ct_percent= round(miss_ct/subject_concepts_mappable_ct,2)) 
  ...
  results_fscore <- 
    ...
    mutate(f_score=((1+beta)*mean_precision*mean_recall)/((beta*mean_precision)+mean_recall))
  rm(precision, recall, mapping_scores_tmp)
  
  # Calculate MMR and MAC scores
